...
import {Home, PlusSquare, User, Heart, Cart} from './icons/index';
      ...
      {state.routes.map((route, index) => {
        ...
        return (
          <TouchableOpacity
            ...
            {label === 'My Shop' && (
              <Cart color={isFocused ? colors.blue : colors.text} size={20} />
            )}
            {label === 'CreatePost' && (
              <PlusSquare color={isFocused ? colors.blue : colors.text} />
            )}
            {label === 'Notification' && (
              <Heart color={isFocused ? colors.blue : colors.text} size={16} />
            )}
            {label === 'User' && (
